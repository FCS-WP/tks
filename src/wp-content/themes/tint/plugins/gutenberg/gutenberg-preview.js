...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			tint_create_observer( 'tint-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				tint_gutenberg_editor_init();
			} );
		} else {
		 	tint_gutenberg_editor_init();
		}
	...
	function tint_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'tint_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'tint_inited' ) ) {
	...
	// Init on page load
	function tint_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = tint_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + TINT_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + TINT_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in TINT_STORAGE['override_classes'] ) {
				$obj.addClass( TINT_STORAGE['override_classes'][i].replace( '%s', TINT_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( tint_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && TINT_STORAGE['sidebar_position'] != 'hide' ) {
				if ( tint_apply_filters( 'tint_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'tint_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( tint_apply_filters( 'tint_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				tint_remove_observer( 'tint-check-editor-styles-wrapper' );
				tint_create_observer( 'tint-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'tint_inited' );
	}
} );
