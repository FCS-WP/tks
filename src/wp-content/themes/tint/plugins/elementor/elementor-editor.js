...
jQuery( document ).ready(
	function() {
		...
		tint_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="tint_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.tint_post_editor' );
						if ( link.length > 0 ) {
							tint_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.tint_post_editor', function(e) {
						tint_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('tint_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function tint_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
