...
jQuery( document ).on( 'action.ready_tint', function() {
	...
	$document.on( 'action.resize_tint', function() {
		_list_products_height = $list_products.length > 0 ? $list_products.height() : 0;
	} );
	...
	// Change display mode
	//--------------------------------
	jQuery( '.woocommerce,.woocommerce-page' ).on( 'click', '.tint_shop_mode_buttons a', function(e) {
		var $self = jQuery(this),
			mode  = $self.hasClass( 'woocommerce_thumbs' ) ? 'thumbs' : 'list';
		tint_set_cookie( 'tint_shop_mode', mode, 365 * 24 * 60 * 60 * 1000 );   // 1 year
	...
	// Add buttons inc/dec
	function tint_woocommerce_add_inc_dec() {
			...
			if ( $self.find('>input[name="quantity"][type="hidden"]').length === 0 ) {
				...
				$self
					.on( 'click', '>span', function(e) {
						tint_woocomerce_inc_dec_click( jQuery( this ) );
						e.preventDefault();
						return false;
					} );
			} else {
	...
	function tint_woocomerce_inc_dec_click(button) {
	...
	$document.on( 'action.scroll_tint', function(e) {
		...
		if ( $list_products.length > 0 && $list_products.offset().top + _list_products_height < tint_window_scroll_top() + tint_window_height() * 1.5 ) {
	...
	jQuery( document.body ).on( 'updated_wc_div', function() {
		...
		tint_create_observer( 'update_woocommerce_cart', table_cart.parents('form').eq(0), function( mutationsList ) {
			for ( var mutation of mutationsList ) {
				if ( mutation.type == 'childList' || mutation.type == 'subtree' ) {
					tint_remove_observer( 'update_woocommerce_cart' );
					table_cart = jQuery( 'table.cart' );
					tint_woocommerce_update_cart( 'update' );
					break;
				}
			}
		} );
	...
	if ( table_cart.length > 0 ) {
		tint_woocommerce_update_cart( 'init' );
	}

	// Resize action
	$window.on( 'resize', function() {
		if ( table_cart.length > 0 ) {
			tint_woocommerce_update_cart( 'resize' );
		}
	} );

	// Update cart display
	function tint_woocommerce_update_cart(status){
		setTimeout(
			function() {
				var w = window.innerWidth;
				if (w == undefined) {
					w = tint_window_width() + ( tint_window_height() < $document.height() || tint_window_scroll_top() > 0 ? 16 : 0 );
				}
				...
				if ( w >= TINT_STORAGE['mobile_layout_width'] && status == 'resize' && jQuery( 'table.cart .mobile_cell' ).length > 0) {
					if ( tbl.length > 0 ) {
