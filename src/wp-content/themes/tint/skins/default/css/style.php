...
if ( ! function_exists( 'tint_customizer_get_css' ) ) {

	function tint_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = tint_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.tint_tabs .tint_tabs_titles li a {
...
CSS;
			$rez          = apply_filters( 'tint_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = tint_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'tint_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( tint_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = tint_get_scheme_colors( $s );
					}
		...
		return apply_filters( 'tint_filter_prepare_css', $css_str, $remove_spaces );
	}
}
