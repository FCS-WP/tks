...
if ( apply_filters( 'tint_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	ob_end_clean();
	$tint_with_featured_image = tint_is_with_featured_image( $tint_post_header /*, array( 'with_gallery' )*/);
	// Post title and meta
	ob_start();
	...
	if ( strpos( $tint_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $tint_with_featured_image ) {
                echo ' with_featured_image' . ( false && tint_get_theme_option( 'single_parallax' ) == 0 ? ' tint-full-height' : '' );
            }
		?>">
			<?php
