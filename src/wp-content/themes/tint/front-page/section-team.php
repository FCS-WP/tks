<div class="front_page_section front_page_section_team<?php
	$tint_scheme = tint_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $tint_scheme ) && ! tint_is_inherit( $tint_scheme ) ) {
		echo ' scheme_' . esc_attr( $tint_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( tint_get_theme_option( 'front_page_team_paddings' ) );
	if ( tint_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$tint_css      = '';
		$tint_bg_image = tint_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$tint_anchor_icon = tint_get_theme_option( 'front_page_team_anchor_icon' );
	$tint_anchor_text = tint_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( tint_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$tint_css      = '';
			$tint_bg_mask  = tint_get_theme_option( 'front_page_team_bg_mask' );
			$tint_bg_color_type = tint_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $tint_bg_color_type ) {
				$tint_bg_color = tint_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$tint_caption = tint_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $tint_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $tint_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $tint_caption, 'tint_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$tint_description = tint_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $tint_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $tint_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $tint_description ), 'tint_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! tint_exists_trx_addons() ) {
						...
						tint_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
